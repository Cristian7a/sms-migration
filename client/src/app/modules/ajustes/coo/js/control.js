...
function cargari_rep(x) {
  var y = x;
  var xmlhttp = new XMLHttpRequest();
  // 1. Apuntamos a la nueva API Python
  var url = "http://localhost:5000/api/reportes/" + y;

  xmlhttp.onreadystatechange = function () {
    if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
      // 2. Recibimos el DTO limpio (ReporteReadDTO)
      // Nota: Si tu API devuelve un objeto único, no necesitas el for,
      // pero si devuelve una lista de 1 elemento, mantenemos la lógica.
      var rep = JSON.parse(xmlhttp.responseText);

      // Adaptador rápido si la API devuelve objeto único en lugar de array
      var array = Array.isArray(rep) ? rep : [rep];

      var i;
      var out = "";
      for (i = 0; i < array.length; i++) {
        // 3. Mapeo de campos: DTO Python -> Vista HTML
        // Nota: 'fecha_evento' viene en formato ISO, 'formato()' podría necesitar ajuste o usarse directo si es YYYY-MM-DD
        var feceve = formato(array[i].fecha_evento);
        var fecrep = formato(
          array[i].fecha_creacion ? array[i].fecha_creacion.split("T")[0] : null
        );

        out +=
          "<tr><td width='30%'><b>Lugar del suceso:</b></td><td width='70%'>" +
          array[i].lugar +
          "</td></tr>" + // Usamos 'lugar' (string limpio del DTO)
          "<tr><td><b>Factor:</b></td><td>" +
          "No disponible (API v2)" +
          "</td></tr>" + // Campo no existente en tu DTO actual
          "<tr><td><b>Causa específica:</b></td><td>" +
          "No disponible (API v2)" +
          "</td></tr>" + // Campo no existente en tu DTO actual
          "<tr><td><b>Frecuencia:</b></td><td>" +
          "No disponible (API v2)" +
          "</td></tr>" + // Campo no existente en tu DTO actual
          "<tr><td><b>Observaciones:</b></td><td>" +
          array[i].descripcion +
          "</td></tr>"; // Usamos 'descripcion' del DTO
      }
    ...
    if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
      ...
      for (i = 0; i < array.length; i++) {
        ...
        out +=
          ...
          "</td></tr><tr><td><b>Observaciones: </b></td><td>" +
          array[i].descripcion +
          "</td></tr>"; // 'descripcion' en lugar de 'OBSREP'
      }
